...
uint8_t flag1 = random(0, 2);             //Bandera para las rutinas del J1
uint8_t flag2 = 0;   //random(0,2);            //Bandera para las rutinas del J2
...
void loop() {
  ...
  if (x2 < 276) {
    ...
    if (posy_J1 == 180) {
      ...
      if (180 - posy_secJ1 < 12) {
        LCD_Print("Game Over", 20, 20, 1, 0x0255, 0x0000);
      }
    }

    //Si la posicion en y del jugador es 154 y cont2 esta en 1
    if (posy_J1 == 154 && cont2 == 1) {

      //Si la posicion en y del jugador esta en el rango del bloque, el jugador pierde
      if (154 - posy_secJ1 < 12) {
          LCD_Print("Game Over", 20, 50, 1, 0x0255, 0x0000);
      }
    }

    //Si la posicion en y del jugador es 128 y cont2 esta en 2
    if (posy_J1 == 128 && cont2 == 2) {

      //Si la posicion en y del jugador esta en el rango del bloque, el jugador pierde
      if (128 - posy_secJ1 < 12) {
        LCD_Print("Game Over", 20, 80, 1, 0x0255, 0x0000);
      }
    }

    //Si la posicion en y del jugador es 102 y cont2 esta en 3
    if (posy_J1 == 102 && cont2 == 3) {

      //Si la posicion en y del jugador esta en el rango del bloque, el jugador pierde
      if (102 - posy_secJ1 < 12) {
        LCD_Print("Game Over", 20, 110, 1, 0x0255, 0x0000);
      }
    }

    //Si la posicion en y del jugador es 76 y cont2 esta en 4
    if (posy_J1 == 76 && cont2 == 4) {

      //Si la posicion en y del jugador esta en el rango del bloque, el jugador pierde
      if (76 - posy_secJ1 < 12) {
        LCD_Print("Game Over", 20, 140, 1, 0x0255, 0x0000);
      }
    }

    //Si la posicion en y del jugador es 50 y cont2 esta en 6
    if (posy_J1 == 50 && cont2 == 6) {

      //Si la posicion en y del jugador esta en el rango del bloque, el jugador pierde
      if (50 - posy_secJ1 < 12) {
        LCD_Print("Game Over", 20, 170, 1, 0x0255, 0x0000);
      }
    }
  }
  ...
  if (digitalRead(SALTAR)) {
    ...
    if (!digitalRead(AGACHARSE)) {
      if ((CONTADOR / 2) % 10 == 0) {
        ...
        if (x2_bala < 270) {
          //FillRect(260, posy_J1 + 5, 16, 6, 0x0255);
        } else {
  ...
  /*
    ...
    if(AparicionBloque1Der == 0 & cont == 0){                               //REVISIÓN DE BANDERA Y CONTADOR
      Plataforma1Der(10, 180);                                              //EJECUCIÓN DE LA FUNCIÓN DEL SPRITE
      if(AparicionBloque1Der == 1){                                         //REVISIÓN SI LA FUNCIÓN HA TERMINADO
        ...
        BalaDer(10, 160);                                                   //EJECUCIÓN DE LA FUNCIÓN DEL SPRITE
        if(AparicionBalaDer == 1){                                          //REVISIÓN SI LA FUNCIÓN HA TERMINADO
          cont = 2;                                                         //AUMENTAR EL CONTADOR
        }
    }if(cont == 2){
      AparicionBalaDer = 0;
      Plataforma2Der(10, 140);
      if(AparicionBloque2Der == 1){
        cont = 3;
      }
    }if(cont == 3){
      AparicionBloque2Der = 0;                                              //LIMPIAR LA BANDERA DE LA ULTIMA FUNCION EJECUTADA
      FillRect(50, 0, 26, 200, 0x0255);
      cont = 0;                                                             //REINICAR EL CONTADOR
      flag1 = random(0,3);                                                  //CAMBIAR DE FORMA ALEATORIA EL VALOR DE LA BANDERA PARA CAMBIAR DE RUTINA
    }
    }
  ...
  */
  ...
  if (flag2 == 0) {                                                         //RUTINA 1
    if (AparicionBloque1Izq == 0 & cont2 == 0) {                            //REVISIÓN DE BANDERA Y CONTADOR
      Plataforma1Izq(20, 180);                                              //EJECUCIÓN DE LA FUNCIÓN DEL SPRITE
      ...
      AparicionBalaIzq = 0;
      Plataforma1Izq(10, 50);
      if (AparicionBloque1Izq == 1) {
        cont2 = 7;
      }
    } if (cont2 == 7) {
      AparicionBloque1Izq = 0;
      FillRect(257, 0, 26, 200, 0x0255);
      posy_J1 = 180;
      posy_secJ1 = 180;
      posy_AgJ1 = posy_J1 + 8;
      cont2 = 0;
